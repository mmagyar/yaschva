import { ValueType, Validation, isArray, isEnum, isObj,
  isString, isMap, isObjectMeta, isNumber } from "./validationTypes";
...
export const validationToType = (input: ValueType | ValueType[]): string => {
  if (Array.isArray(input))
    return input.map(validationToType).join(" | ");

  if (typeof input === "string")
    return simpleTypes(input);
  ...
  if (isEnum(input))
    return input.$enum.map(x => `"${x}"`).join(" | ");
  ...
  if (isString(input))
    return validationToType("string");

  if (isMap(input))
    return `{ [key: string] : ${validationToType(input.$map)}}`;

  if (isObjectMeta(input))
    return validationToType(input.$object);


  if (isNumber(input))
    return validationToType("number");
