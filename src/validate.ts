...
const validateString = (value: InputTypes, enums?: string[]): SimpleValidation => {
  ...
  else if (enums && enums.length && !enums.find(x => value === x))
    return `Value needs to be one of the following: [${enums.join(", ")}] `;
  return null;
};
...
const validateStringObject = (value: InputTypes, validator: StringType): SimpleValidation => {
  if (typeof value !== "string") return "Value is not a string";
  if (validator.$string.minLength && value.length < validator.$string.minLength)
    return "String is shorter than the required minimum length";
  if (validator.$string.maxLength && value.length > validator.$string.maxLength)
    return "String is longer than the required maximum length";
...
export const validate = (type: Validation, value: InputTypes): ValidationResult => {
  ...
  if (Array.isArray(type))
    return validateOneOf(value, type, validate);

  if (isArray(type))
    return validateArray(value, type, validate);

  if (isEnum(type))
    return toResult(validateString(value, type.$enum), value);

  if (isObj(type))
    return validateObject(value, type, validate);

  if (isObjectMeta(type))
    return validateObject(value, type.$object, validate);

  if (isMap(type))
    return validateMap(value, type.$map, validate);
