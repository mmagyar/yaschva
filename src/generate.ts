...
export const generate = (type: Validation,
  ...
  if (Array.isArray(type))
    return generate(type[randomNumber(true, 0, type.length - 1)]);
  ...
  if (isEnum(type))
    return type.$enum[randomNumber(true, 0, type.$enum.length - 1)];
  ...
  if (isNumber(type))
    return randomNumber(false, type.$number.min, type.$number.max);

  if (isMeta(type))
    return simpleGeneration(type.$type);

  if (isString(type)) {
    if (type.$string.regex)
      return randexp(type.$string.regex);
